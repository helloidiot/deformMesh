...
public static class Noise {
	...
	// Hash function to apply seemingly random values to each stripes
	...
	// 1D value nosie
	// Determines two hash values, interpolates between them using the smooth function, and scales the result.
	public static NoiseSample Value1D (Vector3 point, float frequency) {
		...
		// // Interpolate between two new hash values
	...
	public static NoiseSample Perlin1D (Vector3 point, float frequency) {
		...
		// Convert the hash values into gradient functions. 
		float g0 = gradients1D[hash[i0] & gradientsMask1D];
		float g1 = gradients1D[hash[i1] & gradientsMask1D];

		// Compute their values at the current point by multiplying with t0 and t1, which can then be interpolated.
		float v0 = g0 * t0;
		float v1 = g1 * t1;
	...
	// Combine two noise samples, the second at double frequency and half strength or amplitude.
